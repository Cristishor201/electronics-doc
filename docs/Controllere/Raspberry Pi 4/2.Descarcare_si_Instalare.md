...
2. <a href="#install">Ubuntu installation</a><br>
...
```
ssh-keygen -t ed25519 -f ~/.ssh/<ssh_fileName>
```

Aici am observat ca cel mai bine este fara `-b 4096` si fara comentariu `-C <my_email>`. Inlocuieste `<ssh_fileName>` cu `id_raspberry` sau ceva similar.<br>
...
**Step 12**<br>
Apasam pe butonul <i>Save private key</i>, si salvam cheia ca `id_raspberry.ppk` sau ceva similar, si o pastram in acelasi folder `.ssh`.
...
Aici dam pe _Next_, iar la acest pop-up apasam pe _yes_.

<img src="_img/2/rpi_14.PNG" alt="rpi_14" /><!-- prompt1 -->
