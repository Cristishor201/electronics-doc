...
<h3 id="setare-id">Setare fstab prin id USB</h3>

Aceasta obtiune este utila pentru cazul in care sunt mai multe USB-urile conectate, dar si de a-l putea conecta la alt port. Pentru ca setarea se va face in functie de id-ul stickului nu de `sda`.
...
<img src="_img/8/stick3.png" alt="stick3" />

iar dupa ce identificam numele stickului, copiem ce scrie la `UUID`. Spre exemplu la *LABEL* scrie `LABEL=KINGSTON` , iar la *UUID* eu voi avea `UUID=38D5-EAB7`.<br>
Pentru copiere selectam textul cu mouse-ul si dam *click-dreapta-mouse*.
...
```
UUID=38D5-EAB7	/media/usbdrv	exfat	uid=backup,gid=backup,umask=0022,sync,auto,nosuid,rw,nouser	0	0
```

Iar in loc de `/dev/sda1` ca pana acum, punem `UUID=38D5-EAB7` (fara ghilimele)
...
**Step 5**<br>
Dupa va trebui sa ne mai conectam inca o data prin ssh.<br>
...
si va aparea ceva similar precum urmatoarea imagine:

<img src="_img/8/stick4.png" alt="stick4" />

Daca la *MOUNTPOINTS* avem `mnt/usbdrv`, pentru `sda1`, atunci inseamna ca am montat cu succes usb-ul dupa id-ul sau.